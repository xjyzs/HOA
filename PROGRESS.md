...
| `build_gen/` | 预生成 ISA/编译器/运行时头文件（Ruby ERB 模板输出） |
...
ArkCompiler 使用 15 阶段 Ruby/ERB 代码生成流水线，从 ISA YAML 生成数千个头文件和源文件。当前使用预生成的 `build_gen/` 输出（与子模块 commit 确定性一致），原因：
- Ruby 3.3 中 `inst_builder_gen.cpp.erb` 有兼容性问题（`undefined method 'key?' for nil`）
- 已修复 7 个 Ruby 脚本的 `require 'ostruct'` 问题
- `build_runtime.sh` 在 `build_gen/` 存在且非空时自动跳过 codegen
...
- 修复 Ruby 3.3 代码生成兼容性（`inst_builder_gen.cpp.erb`）
